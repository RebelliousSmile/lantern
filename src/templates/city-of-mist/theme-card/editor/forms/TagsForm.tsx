...
export default function TagsForm({
    ...
    const tags =
        field === 'power'
            ? cityOfMistThemeCard.power_tags
            : cityOfMistThemeCard.weakness_tags
    const ids = useMemo(
        () => tags.map((tag, index) => `${index}::${tag.text}`),
        [tags]
    )
        ...
        <div className="space-y-2.5">
            <div className="flex items-baseline justify-between gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                    Letters are optional for homebrew tags.
                </p>
            </div>
            <DndContext
                sensors={sensors}
                    ...
                    <ul className="space-y-2">
                        {tags.map((tag, index) => (
                            <TagRow
                                ...
                                onUpdate={(update) =>
                                    updateTagAt(field, index, update)
                                }
            ...
            <p className="text-xs text-muted-foreground">
                During play, an extra power tag may temporarily exist before its
                matching weakness is added.
            </p>
        </div>
    )
}
    ...
    return (
        ...
        >
            <div className="grid grid-cols-[auto_3.5rem_minmax(0,1fr)_auto] items-end gap-2">
                <button
                    type="button"
                    className="mb-0.5 inline-flex h-7 w-7 cursor-grab items-center justify-center rounded hover:bg-muted"
                ...
                <div className="grid gap-1">
                    <Label htmlFor={`${id}-letter`} className="text-xs">
                        Letter
                    </Label>
                    <Input
                        id={`${id}-letter`}
            ...
            <div className="mt-2 flex items-center justify-end gap-2">
                ...
                <Switch
                    id={`${id}-state`}
                    checked={
                        field === 'power' ? !!tag.is_burnt : !!tag.is_invoked
                    }
