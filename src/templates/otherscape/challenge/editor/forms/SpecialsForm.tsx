...
export default function SpecialsForm({ focusIndex }: { focusIndex?: number }) {
    ...
    useEffect(() => {
        if (
            ...
            otherscapeChallenge.specials[focusIndex]
        ) {
    ...
    const itemIds = useMemo(
        () =>
            otherscapeChallenge.specials.map(
                (sf, i) => `${i}::${sf.name || 'special'}`
            ),
        [otherscapeChallenge.specials]
    )
    ...
    function uniquePlaceholderName(): string {
        ...
        const used = new Set(
            otherscapeChallenge.specials.map((sf) =>
                (sf.name || '').toLowerCase()
            )
        )
    ...
    function addPlaceholder() {
        ...
        const newIndex = otherscapeChallenge.specials.length
    ...
    function startEdit(i: number) {
        const sf = otherscapeChallenge.specials[i]
        if (!sf) return
        setEditingIndex(i)
        setEName(sf.name || '')
            ...
            >
                ...
                >
                    <ul className="space-y-1.5">
                        {otherscapeChallenge.specials.map((sf, i) => (
                            <SortableSpecialItem
                                key={itemIds[i]}
                                id={itemIds[i]}
                                name={sf.name}
