...
import { useOtherscapeThemeKitStore, type TagField } from '../../hooks'

/* Tags are stored bare; the braces only exist for display, so the preview
   formatter is reused here to show what the card will read like. */
...
export default function TagsForm({
    ...
    const { otherscapeThemeKit, addTag, removeTagAt, replaceTagAt, moveTag } =
    ...
    // Reaching this form from an empty field opens straight into a new entry
    useEffect(() => {
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        if (from >= 0 && to >= 0 && from !== to) moveTag(field, from, to)
    }

    const dragDisabled = editingIndex !== null

    function addPlaceholder() {
        const samples = PLACEHOLDERS[field]
    ...
    function confirmEdit() {
        ...
        const next = raw.trim()
        if (!next) {
            setError(text('otherscape:forms.themeKit.tags.emptyValueError'))
            return
        }

        replaceTagAt(field, editingIndex, next)
        cancelEdit()
    }
        ...
        <div className="space-y-2.5">
            <div className="flex items-baseline justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    {text(`otherscape:forms.themeKit.tags.fieldLabel.${field}`)}
                </p>
                <p className="text-xs text-muted-foreground">
                    {text('otherscape:forms.themeKit.tags.hint')}
                </p>
            </div>
                    ...
                    <ul className="space-y-1.5">
                                ...
                                {editingIndex === index && (
                                    <div className="mt-2 space-y-2.5 rounded-md border bg-muted/30 p-2.5">
                                        ...
                                        <div className="grid gap-1">
                                            <Label
                                                htmlFor={`os-theme-kit-tag-${field}-${index}`}
                                                className="text-xs"
                                            >
                                                {text(
                                                    'otherscape:forms.themeKit.tags.tagLabel'
                                                )}
                                            </Label>
                                            <Input
                                                id={`os-theme-kit-tag-${field}-${index}`}
                                                ...
                                                placeholder={text(
                                                    'otherscape:forms.themeKit.tags.tagInputPlaceholder'
                                                )}
                                            />
                                        </div>
                        ...
                        <li className="flex">
                            ...
                            >
                                ...
                                {text('otherscape:forms.themeKit.tags.addTag')}
                            </Button>
                        </li>
                    </ul>
                </SortableContext>
                  ...
                  dragDisabled
                      ...
                      : 'cursor-grab active:cursor-grabbing'
                        ...
                        title={
                            dragDisabled
                                ? text(
                                      'otherscape:forms.themeKit.tags.finishEditingToReorder'
                                  )
                    ...
                    >
                        <span
                            ...
                            dangerouslySetInnerHTML={{
                                __html: renderLitmInline(FORMAT[field](value)),
                            }}
                            title={value}
                        />
                    </SystemMarkdownScope>
