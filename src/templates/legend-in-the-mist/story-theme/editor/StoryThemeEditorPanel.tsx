import { useLegendInTheMistStoryThemeSheetStore } from '../hooks'
import BasicForm from './forms/BasicForm'
import MetaForm from './forms/MetaForm'
import QuestForm from './forms/QuestForm'
import TagsForm from './forms/TagsForm'

export function StoryThemeEditorPanel() {
    const { open, target } = useLegendInTheMistStoryThemeSheetStore()
        ...
        case 'tags':
            return (
                /* Remounting on the field keeps each list's inline editor to
                   itself when the preview jumps from one field to the other. */
                <TagsForm
                    key={target.field}
                    field={target.field}
